...
/* ─── Расчёты скоринга ───
   ...
   • Анонимные расчёты живут ограниченное время (expiresAt) и удаляются.
   • Расчёты залогиненного пользователя живут, пока он их не удалит (история).
...
export const scoringCalculation = pgTable(
  "scoring_calculation",
  {
    ...
    ipHash: text("ip_hash"),
    createdAt: timestamp("created_at").notNull().defaultNow(),
    /** Срок жизни анонимного расчёта; null — хранится бессрочно (есть владелец). */
