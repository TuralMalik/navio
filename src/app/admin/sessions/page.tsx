...
export default async function SessionsPage({
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Sessiyalar"
        subtitle="Bir sessiya = bir ziyarət. 30 dəqiqə hərəkətsizlikdən sonra yenisi başlayır."
        right={<RangeTabs days={days} base="/admin/sessions" />}
      />

      <Panel title={`${sessions.length} sessiya`} subtitle="Sətrə klikləyib bütün yolu görün" pad={false}>
        {sessions.length === 0 ? (
          <div className="p-4"><Empty what="sessiya" /></div>
        ) : (
          <Table head={["Başlanğıc", "Giriş səhifəsi", "Baxış", "Hadisə", "Aktiv vaxt", "Mənbə", "Ölkə", "Kim", ""]}>
            {sessions.map((s) => (
