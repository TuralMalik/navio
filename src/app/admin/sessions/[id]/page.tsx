...
export default async function SessionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Panel title="Path" subtitle="Page views and events in chronological order">
              ...
              <div className="flex items-baseline gap-2 flex-wrap">
                ...
                {item.kind === "view" ? (
                  <>
                    ...
                    {item.isFirst && <Badge tone="blue">entry</Badge>}
                    {item.durationMs > 0 && (
                      <span className="text-[11.5px] text-slate-500">{fmtDuration(item.durationMs)} active</span>
                    )}
                  </>
                ) : (
                  <>
